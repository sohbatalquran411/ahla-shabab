...
const params = useParams()
  ...
  const deleteForm = async () => {
    ...
    if (!confirm('هل أنت متأكد من رغبتك في حذف هذا النموذج بالكامل؟ لا يمكن التراجع عن هذه الخطوة وسيتم حذف جميع الردود والأسئلة المرتبطة به.')) return
    ...
    try {
      ...
      if (count === 0) {
        throw new Error('لم يتم الحذف. لا توجد صلاحيات RLS للحذف أو النموذج غير موجود.')
      }
      ...
      alert('حدث خطأ أثناء محاولة الحذف: تأكد من صلاحيات قاعدة البيانات (RLS).')
    } finally {
      setSaving(false)
    }
  }
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                {responseCount > 0 && (profile?.role === 'admin' || profile?.role === 'supervisor') && (
                  <>
                    ...
                    <a
                      ...
                      className="flex-1 sm:flex-none px-4 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-colors font-bold text-sm sm:text-base flex justify-center items-center gap-2 text-center shadow-lg shadow-green-200"
                    >
            ...
            <div className="flex w-full sm:w-auto items-center gap-3">
              <button
                onClick={deleteForm}
                disabled={saving}
                className="w-full sm:w-auto px-6 py-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 font-bold text-base shadow-sm border border-red-200"
                title="حذف النموذج نهائياً"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
